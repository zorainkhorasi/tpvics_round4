...
<style>
    /* ===================================================
       ...
       (From your first post - slightly modified for selects)
       =================================================== */
    ...
    /* Primary labels for dynamically generated fields (to match old code style) */
    ...
    /* 3. Header Cluster/Area Box (Teal Match) */
    .info-header-box {
        width: 100%;
    ...
    .info-header-box strong { font-weight: bold; color: #000000; }
    ...
    /* Adjusted sub-field structure within detail boxes to support inline labels */
    .detail-box .mb-3,
    .detail-box .mb-0 {
        display: flex;
        align-items: center;
    ...
    /* This class is used for all dynamic inputs for the sleek look */
    ...
    .form-control-underline:focus {
        ...
        border-right: none;
        border-radius: 0;
        /* Exact line color for the input underline (matches image) */
    ...
    /* Error styling (used in old JS) */
    .form-control.error, .form-control-plaintext.error {
        border-color: red !important;
    }

    /* 6. Totals Section - We are using the searchData dynamic HTML for this now,
          but keeping the original totals styling for consistency */
    .totals-section > div {
        display: flex;
        align-items: center;
    }
    ...
    /* Styles for the old logic's required classes */
    .hide { display: none !important; }
    .geoarea_name { margin: 0; padding-top: 5px; font-size: 0.9rem; font-weight: bold; }
    ...
    /* Dynamic table structure container style */
    .dynamic-table-row {
        margin-bottom: 15px; /* Spacing between dynamically generated rows */
        padding: 10px;
        border: 1px solid #e9ecef;
        background-color: #f7f7f7;
    }
    .dynamic-table-row .form-group {
        margin-bottom: 5px; /* Tighten up spacing within the dynamic rows */
    }
                ...
                <div class="row mb-3 align-items-end">
                    <div class="col-md-3 d-flex align-items-center">
                        ...
                        <div class="flex-grow-1">
                            <select class="form-control form-control-underline select2 district_select" autocomplete="uc"
                    ...
                    <div class="col-md-3 d-flex align-items-center">
                        ...
                        <div class="flex-grow-1">
                            <select class="form-control form-control-underline select2 clusters_select"
                    ...
                    <div class="col-md-6 d-flex flex-column justify-content-md-end justify-content-start mt-3 mt-md-0">
                        <div class="info-header-box mb-2">
                            ...
                            Area: <span id="area_display">TBD</span> | <span id="village_display">TBD</span>
...
<script>
    // NOTE: This entire script block is copied from your provided old logic.
    // The ONLY changes are inside searchData() where the HTML generation is updated
    // to use the Bootstrap 5 inline/underline styles (form-control-plaintext and column classes).
    ...
    $(document).ready(function () {
        //changeDist(); // This function is commented out in your original code
        mydate();
    });
        ...
        if (flag == 0) {
            ...
            CallAjax('<?php echo base_url() . 'index.php/Manual_linelisting/checkClusterExists' ?>',
                data, 'POST', function (res) {
                    ...
                    // Reset styling before checking results
                    $('#district_select').css('border', '');
                    $('#clusters_select').css('border', '');
                    ...
                    if (response.exists == 0) {
                        ...
                        if (c.geoarea !== undefined && c.geoarea !== '') {
                            // Update the info box using the new UI's element IDs
                            $('#cluster_id_display').text(cluster_select);
                            // Assuming geoarea contains Area and Village separated by | or similar
                            // We can't parse it reliably, so we use the geoarea_name class for the text from old code.
                            $('.geoarea_name').html('Cluster: <strong>' + cluster_select + '</strong> (Area: ' + c.geoarea + ')');

                            // You might want to update the visible cluster box too for more visibility
                            $('#area_display').text(c.geoarea.split('|')[0] || 'N/A');
                            $('#village_display').text(c.geoarea.split('|')[1] || 'N/A');
    ...
    function searchData() {
        ...
        if (flag == 0) {
            ...
            // 1. DYNAMIC TOTALS FIELDS (Using NEW UI structure)
            html += '<div class="row totals-section mb-4">';

            // Total Structure Identified
            html += '<div class="col-md-3 mb-3 d-flex align-items-center"> ' +
            ...
            // Total Household having 12-23 Months Children
            html += '<div class="col-md-3 mb-3 d-flex align-items-center"> ' +
                '   <label for="total_household_identified" class="label-inline me-2">Total Household having 12-23 Months Children</label>' +
                '   <div class="flex-grow-1">' +
                '       <input type="number" maxlength="3" max="3" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==3) return false;"  minlength="1" min="1" class="form-control form-control-plaintext total_household_identified"  autocomplete="total_hh" value="' + number_hh + '"  ' +
            ...
            html += '<div class="col-md-3 mb-3 d-flex align-items-center"> ' +
                ...
                '   <div class="flex-grow-1">' +
                '       <input type="number" maxlength="3" max="3" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==3) return false;"  minlength="1" min="1" class="form-control form-control-plaintext total_residential_structures"  autocomplete="total_hs"  ' +
            ...
            // 2. DYNAMIC LINELISTING ROWS (Using NEW UI input style within the old loop)
            html += '<h5 class="mt-4 mb-3 text-secondary">Household Linelisting Details</h5>';

            for (var i = 1; i <= number_hh; i++) {
                // Use a dynamic-table-row wrapper for better spacing/visual separation of each HH block
                html += `<div class="row dynamic-table-row" id="hh_row_${i}">`;

                // Structure Number
                html += '<div class="col-sm-3 col-6"> ' +
                    '       <div class="form-group">' +
                    '            <label for="structure_number_' + i + '" class="label-control text-primary">Structure Number</label>' +
                    '            <input type="number" maxlength="3" max="3" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==3) return false;" minlength="1" min="1" class="form-control form-control-underline structure_number_' + i + '"  autocomplete="structure_number" id="structure_number_' + i + '"  required>' +
                    '        </div>' +
                    '    </div> ';
                ...
                html += '<div class="col-sm-3 col-6"> ' +
                    '       <div class="form-group">' +
                    '            <label for="household_no_' + i + '" class="label-control text-primary">S.No of Household</label>' +
                    '            <input type="number" maxlength="3" max="3" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==3) return false;" minlength="1" min="1" class="form-control form-control-underline household_no_' + i + '"   autocomplete="household_no" id="household_no_' + i + '"  required>' +
                    '        </div>' +
                    '    </div> ';
                ...
                html += '<div class="col-sm-3 col-6"> ' +
                    ...
                    '            <input type="text" class="form-control form-control-underline household_name_' + i + '" id="household_name_' + i + '"  autocomplete="name_hh" required>' +
                    '        </div>' +
                    '    </div> ';

                // Total 12-23 months children
                html += '<div class="col-sm-3 col-6"> ' +
                    '       <div class="form-group">' +
                    '              <label for="childAge_' + i + '" class="label-control text-primary">Total 12-23 months children</label>' +
                    '              <input type="number" maxlength="1" max="1" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==1) return false;" minlength="1" min="1" class="form-control form-control-underline childAge_' + i + '"  autocomplete="childAge" id="childAge_' + i + '"  required>' +
                    '        </div>' +
                    '    </div> ';
                html += '</div>'; // End of dynamic-table-row
            }

            // 3. SAVE BUTTON AND NOTE
            html += '<div class="text-right mt-4">' +
                '<button type="button" class="btn btn-info mybtn" onclick="submitLineListing()">' +
                'Save Data</button>' +
                '</div>';
    ...
    // The rest of the JS functions remain identical to your provided code to ensure all validation logic works
    function mydate() {
    ...
    // submitLineListing function is kept here, exactly as provided,
    // relying on the IDs and classes set by the modified searchData()
    function submitLineListing() {
        ...
        if (data['vaccinator_frequency'] == "6") {
            ...
            // NOTE: The date validation here is comparing HTML input type='date' value (yyyy-mm-dd)
            // against today's date in yyyy-mm-dd format.
            if (visitDate >= today) {
                $('#vaccinator_visit_date').css('border', '1px solid red');
                flag = 1;
